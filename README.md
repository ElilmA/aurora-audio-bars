...
捕获 Windows 系统音频，将实时频谱渲染为屏幕左右两侧的彩色竖向频谱条（1mm 宽、全屏高、透明置顶、点击穿透）。
...
- 屏幕左右两侧彩色竖向频谱条：宽约 1mm、高 = 虚拟屏幕全高，频带颜色渐变，随音频律动
- 透明置顶、点击穿透（WS_EX_TRANSPARENT + LAYERED + TOOLWINDOW + NOACTIVATE），右键托盘完整控制
- 多显示器（左右条自动贴虚拟屏幕最左/最右边缘）+ Per-Monitor V2 DPI 感知
- 托盘菜单：启用/禁用、强度、透明度、开机自启、退出
...
运行后无主窗口，仅托盘图标 + 屏幕两侧频谱条。播放任意音频即可看到律动。
